...
import BooleanViz from '@/components/BooleanViz'
        ...
        <main style={{ flex: 1, minWidth: 0 }}>
          <div className="concept-header" style={{ padding: '32px 48px 0', borderBottom: '1px solid var(--rule)' }}>
            <div className="mono" style={{ fontSize: 11, color: 'var(--ink-faint)', marginBottom: 12, letterSpacing: '0.04em' }}>
              <Link href="/subjects" style={{ color: 'var(--ink-mute)', textDecoration: 'none' }}>Subjects</Link>{' > '}<span style={{ color: 'var(--ink-mute)' }}>Digital Circuits</span>{' > '}<span>Boolean Algebra</span>
            </div>
            <div className="concept-title-area" style={{ display: 'flex', alignItems: 'start', justifyContent: 'space-between', marginBottom: 16 }}>
              <div>
                ...
                <h1 className="serif" style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 400, lineHeight: 1.1 }}>Boolean Algebra</h1>
              </div>
            ...
            <div className="concept-tags" style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
              <span className="pill pill--teal">ECE Core</span><span className="pill">1-2 marks</span><span className="pill pill--accent">MCQ</span><span className="pill pill--warn">Appears in 5/7 GATE papers</span>
            </div>
            <div className="tabs">
              {TABS.map((tab, i) => (
          ...
          <div className="concept-main" style={{ padding: '0 48px 80px' }} key={activeTab}>
            ...
            {activeTab === 1 && <BooleanViz />}
            {activeTab === 2 && <GenericLabTab lab={BOOL_LAB} concept="boolean" />}
            {activeTab === 3 && <PYQsTab pyqs={BOOL_PYQS} insight={BOOL_INSIGHT} conceptTitle="Boolean Algebra" />}
            {activeTab === 4 && <PracticeTab practice={BOOL_PRACTICE} conceptTitle="Boolean Algebra" />}
          </div>
        </main>
      </div>
    </>
