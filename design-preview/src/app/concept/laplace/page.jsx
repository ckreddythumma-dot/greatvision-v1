...
import LaplaceViz from '@/components/LaplaceViz'
import GenericLabTab from '@/components/GenericLabTab'
import PYQsTab from '@/components/PYQsTab'
import PracticeTab from '@/components/PracticeTab'
import { LAPLACE_THEORY, LAPLACE_LAB, LAPLACE_PYQS, LAPLACE_PRACTICE, LAPLACE_INSIGHT } from '@/data/laplace'
        ...
        <aside className="csidebar">
          ...
          {concepts.map((c, i) => (
            <Link key={c.id} href={conceptRoutes[c.id] || '#'} style={{ textDecoration: 'none', color: 'inherit' }}>
              <div className={`csidebar__item ${c.id === 'laplace' ? 'is-active' : ''}`}>
        ...
        <main style={{ flex: 1, minWidth: 0 }}>
          <div className="concept-header" style={{ padding: '32px 48px 0', borderBottom: '1px solid var(--rule)' }}>
            <div className="mono" style={{ fontSize: 11, color: 'var(--ink-faint)', marginBottom: 12, letterSpacing: '0.04em' }}>
              <Link href="/subjects" style={{ color: 'var(--ink-mute)', textDecoration: 'none' }}>Subjects</Link>{' > '}<span style={{ color: 'var(--ink-mute)' }}>Mathematics</span>{' > '}<span>Laplace Transform</span>
            </div>
            <div className="concept-title-area" style={{ display: 'flex', alignItems: 'start', justifyContent: 'space-between', marginBottom: 16 }}>
              <div>
                ...
                <h1 className="serif" style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 400, lineHeight: 1.1 }}>Laplace Transform</h1>
              </div>
          ...
          <div className="concept-main" style={{ padding: '0 48px 80px' }} key={activeTab}>
            {activeTab === 0 && <TheoryTab theory={LAPLACE_THEORY} />}
            {activeTab === 1 && <LaplaceViz />}
            {activeTab === 2 && <GenericLabTab lab={LAPLACE_LAB} concept="laplace" />}
            {activeTab === 3 && <PYQsTab pyqs={LAPLACE_PYQS} insight={LAPLACE_INSIGHT} conceptTitle="Laplace Transform" />}
            {activeTab === 4 && <PracticeTab practice={LAPLACE_PRACTICE} conceptTitle="Laplace Transform" />}
          </div>
        </main>
      </div>
    </>
