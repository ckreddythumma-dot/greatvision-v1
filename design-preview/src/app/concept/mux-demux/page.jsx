...
import MuxViz from '@/components/MuxViz'
import GenericLabTab from '@/components/GenericLabTab'
import PYQsTab from '@/components/PYQsTab'
import PracticeTab from '@/components/PracticeTab'
import { MUX_THEORY, MUX_LAB, MUX_PYQS, MUX_PRACTICE, MUX_INSIGHT } from '@/data/mux-demux'
        ...
        <aside className="csidebar">
          ...
          {concepts.map((c, i) => (
            <Link key={c.id} href={conceptRoutes[c.id] || '#'} style={{ textDecoration: 'none', color: 'inherit' }}>
              <div className={`csidebar__item ${c.id === 'mux-demux' ? 'is-active' : ''}`}>
        ...
        <main style={{ flex: 1, minWidth: 0 }}>
          <div className="concept-header" style={{ padding: '32px 48px 0', borderBottom: '1px solid var(--rule)' }}>
            <div className="mono" style={{ fontSize: 11, color: 'var(--ink-faint)', marginBottom: 12, letterSpacing: '0.04em' }}>
              <Link href="/subjects" style={{ color: 'var(--ink-mute)', textDecoration: 'none' }}>Subjects</Link>{' > '}<span style={{ color: 'var(--ink-mute)' }}>Digital Circuits</span>{' > '}<span>MUX / DEMUX</span>
            </div>
            <div className="concept-title-area" style={{ display: 'flex', alignItems: 'start', justifyContent: 'space-between', marginBottom: 16 }}>
              <div>
                ...
                <h1 className="serif" style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 400, lineHeight: 1.1 }}>MUX / DEMUX</h1>
              </div>
          ...
          <div className="concept-main" style={{ padding: '0 48px 80px' }} key={activeTab}>
            {activeTab === 0 && <TheoryTab theory={MUX_THEORY} />}
            {activeTab === 1 && <MuxViz />}
            {activeTab === 2 && <GenericLabTab lab={MUX_LAB} concept="mux-demux" />}
            {activeTab === 3 && <PYQsTab pyqs={MUX_PYQS} insight={MUX_INSIGHT} conceptTitle="MUX / DEMUX" />}
            {activeTab === 4 && <PracticeTab practice={MUX_PRACTICE} conceptTitle="MUX / DEMUX" />}
          </div>
        </main>
      </div>
    </>
