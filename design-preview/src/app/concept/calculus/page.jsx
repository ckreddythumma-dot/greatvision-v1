...
import CalculusViz from '@/components/CalculusViz'
import GenericLabTab from '@/components/GenericLabTab'
import PYQsTab from '@/components/PYQsTab'
import PracticeTab from '@/components/PracticeTab'
import { CALC_THEORY, CALC_LAB, CALC_PYQS, CALC_PRACTICE, CALC_INSIGHT } from '@/data/calculus'
        ...
        <aside className="csidebar">
          ...
          {concepts.map((c, i) => (
            <Link key={c.id} href={conceptRoutes[c.id] || '#'} style={{ textDecoration: 'none', color: 'inherit' }}>
              <div className={`csidebar__item ${c.id === 'calculus' ? 'is-active' : ''}`}>
        ...
        <main style={{ flex: 1, minWidth: 0 }}>
          <div className="concept-header" style={{ padding: '32px 48px 0', borderBottom: '1px solid var(--rule)' }}>
            <div className="mono" style={{ fontSize: 11, color: 'var(--ink-faint)', marginBottom: 12, letterSpacing: '0.04em' }}>
              <Link href="/subjects" style={{ color: 'var(--ink-mute)', textDecoration: 'none' }}>Subjects</Link>{' > '}<span style={{ color: 'var(--ink-mute)' }}>Mathematics</span>{' > '}<span>Calculus</span>
            </div>
            <div className="concept-title-area" style={{ display: 'flex', alignItems: 'start', justifyContent: 'space-between', marginBottom: 16 }}>
              <div>
                ...
                <h1 className="serif" style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 400, lineHeight: 1.1 }}>Calculus (Maxima/Minima)</h1>
              </div>
          ...
          <div className="concept-main" style={{ padding: '0 48px 80px' }} key={activeTab}>
            {activeTab === 0 && <TheoryTab theory={CALC_THEORY} />}
            {activeTab === 1 && <CalculusViz />}
            {activeTab === 2 && <GenericLabTab lab={CALC_LAB} concept="calculus" />}
            {activeTab === 3 && <PYQsTab pyqs={CALC_PYQS} insight={CALC_INSIGHT} conceptTitle="Calculus" />}
            {activeTab === 4 && <PracticeTab practice={CALC_PRACTICE} conceptTitle="Calculus" />}
          </div>
        </main>
      </div>
    </>
