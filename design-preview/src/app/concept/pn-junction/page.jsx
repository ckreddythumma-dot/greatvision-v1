...
import PNJunctionViz from '@/components/PNJunctionViz'
import GenericLabTab from '@/components/GenericLabTab'
import PYQsTab from '@/components/PYQsTab'
import PracticeTab from '@/components/PracticeTab'
import PNJunctionAnalogy from '@/components/PNJunctionAnalogy'
import { PN_THEORY, PN_LAB, PN_PYQS, PN_PRACTICE, PN_INSIGHT } from '@/data/pn-junction'
      ...
      <div className="page-shell" style={{ display: 'flex', minHeight: '100dvh' }}>
        <aside className="csidebar">
          ...
          {concepts.map((c, i) => (
            <Link key={c.id} href={conceptRoutes[c.id] || '#'} style={{ textDecoration: 'none', color: 'inherit' }}>
              <div className={`csidebar__item ${c.id === 'pn-junction' ? 'is-active' : ''}`}>
        ...
        <main style={{ flex: 1, minWidth: 0 }}>
          ...
          <div className="concept-main" style={{ padding: '0 48px 80px' }} key={activeTab}>
            {activeTab === 0 && <TheoryTab theory={PN_THEORY} Analogy={PNJunctionAnalogy} />}
            {activeTab === 1 && <PNJunctionViz />}
            {activeTab === 2 && <GenericLabTab lab={PN_LAB} concept="pn-junction" />}
            {activeTab === 3 && <PYQsTab pyqs={PN_PYQS} insight={PN_INSIGHT} conceptTitle="PN Junction" />}
            {activeTab === 4 && <PracticeTab practice={PN_PRACTICE} conceptTitle="PN Junction" />}
          </div>
        </main>
      </div>
    </>
