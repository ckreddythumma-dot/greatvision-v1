...
import DiffEqViz from '@/components/DiffEqViz'
import GenericLabTab from '@/components/GenericLabTab'
import PYQsTab from '@/components/PYQsTab'
import PracticeTab from '@/components/PracticeTab'
import { DE_THEORY, DE_LAB, DE_PYQS, DE_PRACTICE, DE_INSIGHT } from '@/data/differential-equations'
        ...
        <aside className="csidebar">
          ...
          {concepts.map((c, i) => (
            <Link key={c.id} href={conceptRoutes[c.id] || '#'} style={{ textDecoration: 'none', color: 'inherit' }}>
              <div className={`csidebar__item ${c.id === 'diff-eq' ? 'is-active' : ''}`}>
        ...
        <main style={{ flex: 1, minWidth: 0 }}>
          <div className="concept-header" style={{ padding: '32px 48px 0', borderBottom: '1px solid var(--rule)' }}>
            <div className="mono" style={{ fontSize: 11, color: 'var(--ink-faint)', marginBottom: 12, letterSpacing: '0.04em' }}>
              <Link href="/subjects" style={{ color: 'var(--ink-mute)', textDecoration: 'none' }}>Subjects</Link>{' > '}<span style={{ color: 'var(--ink-mute)' }}>Mathematics</span>{' > '}<span>Differential Equations</span>
            </div>
            <div className="concept-title-area" style={{ display: 'flex', alignItems: 'start', justifyContent: 'space-between', marginBottom: 16 }}>
              <div>
                ...
                <h1 className="serif" style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 400, lineHeight: 1.1 }}>Differential Equations</h1>
              </div>
            ...
            <div className="concept-tags" style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
              <span className="pill pill--teal">ECE Core</span><span className="pill">2 marks</span><span className="pill pill--accent">NAT + MCQ</span><span className="pill pill--warn">Appears in 4/7 GATE papers</span>
            </div>
            <div className="tabs">
              {TABS.map((tab, i) => (
          ...
          <div className="concept-main" style={{ padding: '0 48px 80px' }} key={activeTab}>
            {activeTab === 0 && <TheoryTab theory={DE_THEORY} />}
            {activeTab === 1 && <DiffEqViz />}
            {activeTab === 2 && <GenericLabTab lab={DE_LAB} concept="diff-eq" />}
            {activeTab === 3 && <PYQsTab pyqs={DE_PYQS} insight={DE_INSIGHT} conceptTitle="Differential Equations" />}
            {activeTab === 4 && <PracticeTab practice={DE_PRACTICE} conceptTitle="Differential Equations" />}
          </div>
        </main>
      </div>
    </>
