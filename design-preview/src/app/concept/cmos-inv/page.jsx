...
import CMOSInvViz from '@/components/CMOSInvViz'
import GenericLabTab from '@/components/GenericLabTab'
import PYQsTab from '@/components/PYQsTab'
import PracticeTab from '@/components/PracticeTab'
import CMOSInvAnalogy from '@/components/CMOSInvAnalogy'
import { CMOS_THEORY, CMOS_LAB, CMOS_PYQS, CMOS_PRACTICE, CMOS_INSIGHT } from '@/data/cmos-inverter'
      ...
      <div className="page-shell" style={{ display: 'flex', minHeight: '100dvh' }}>
        <aside className="csidebar">
          ...
          {concepts.map((c, i) => (
            <Link key={c.id} href={conceptRoutes[c.id] || '#'} style={{ textDecoration: 'none', color: 'inherit' }}>
              <div className={`csidebar__item ${c.id === 'cmos-inv' ? 'is-active' : ''}`}>
        ...
        <main style={{ flex: 1, minWidth: 0 }}>
          ...
          <div className="concept-main" style={{ padding: '0 48px 80px' }} key={activeTab}>
            {activeTab === 0 && <TheoryTab theory={CMOS_THEORY} Analogy={CMOSInvAnalogy} />}
            {activeTab === 1 && <CMOSInvViz />}
            {activeTab === 2 && <GenericLabTab lab={CMOS_LAB} concept="cmos-inv" />}
            {activeTab === 3 && <PYQsTab pyqs={CMOS_PYQS} insight={CMOS_INSIGHT} conceptTitle="CMOS Inverter" />}
            {activeTab === 4 && <PracticeTab practice={CMOS_PRACTICE} conceptTitle="CMOS Inverter" />}
          </div>
        </main>
      </div>
    </>
