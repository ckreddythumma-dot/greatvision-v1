...
import MosfetAmpViz from '@/components/MosfetAmpViz'
import GenericLabTab from '@/components/GenericLabTab'
import PYQsTab from '@/components/PYQsTab'
import PracticeTab from '@/components/PracticeTab'
import MosfetAmpAnalogy from '@/components/MosfetAmpAnalogy'
import { AMP_THEORY, AMP_LAB, AMP_PYQS, AMP_PRACTICE, AMP_INSIGHT } from '@/data/mosfet-amplifier'
...
export default function MosfetAmpPage() {
        ...
        <aside className="csidebar">
          ...
          {concepts.map((c, i) => (
            <Link key={c.id} href={conceptRoutes[c.id] || '#'} style={{ textDecoration: 'none', color: 'inherit' }}>
              <div className={`csidebar__item ${c.id === 'mosfet-amp' ? 'is-active' : ''}`}>
        ...
        <main style={{ flex: 1, minWidth: 0 }}>
          <div className="concept-header" style={{ padding: '32px 48px 0', borderBottom: '1px solid var(--rule)' }}>
            ...
            <div className="concept-tags" style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
              ...
              <span className="pill pill--warn">Appears in 4/7 GATE papers</span>
            </div>
          ...
          <div className="concept-main" style={{ padding: '0 48px 80px' }} key={activeTab}>
            {activeTab === 0 && <TheoryTab theory={AMP_THEORY} Analogy={MosfetAmpAnalogy} />}
            {activeTab === 1 && <MosfetAmpViz />}
            {activeTab === 2 && <GenericLabTab lab={AMP_LAB} concept="mosfet-amp" />}
            {activeTab === 3 && <PYQsTab pyqs={AMP_PYQS} insight={AMP_INSIGHT} conceptTitle="MOSFET Amplifier" />}
            {activeTab === 4 && <PracticeTab practice={AMP_PRACTICE} conceptTitle="MOSFET Amplifier" />}
          </div>
        </main>
      </div>
    </>
